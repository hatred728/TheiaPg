...
DECLSPEC_NORETURN VOID DieDispatchIntrnlError(IN ULONG32 InternalCode)
{
	...
	if (!(_interlockedbittestandset(&SynchBarrier0, 0I32)))
	{
		...
		else
		{
			...
			if (!__readcr8()) 
			{
				HrdSetSPx64(g_pStackAddrRetAddrImgEntry);
			}
			else 
			{ 
				DbgLog("[TheiaPg <->] DieDispatchIntrnlError: Soft method is canceled, current core possibly executed in the context ISR | IRQL : 0x%02X\n", __readcr8());

				goto ExitDieSoftMethod;
			}
		}
	}
	else
